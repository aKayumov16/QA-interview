...
- **Эксплуатация → Поддержка**: продукт живёт после релиза, и команда должна быть готова реагировать на изменения.
      ...
      <details style='margin-left: 20px'>
    <summary style='font-size: 16px'>Жизненный цикл бага</summary>
    <p style='font-size: 14px'>

## Жизненный цикл бага (простой обзор)

| Шаг | Что происходит | Кто обычно отвечает |
|-----|----------------|---------------------|
| **1. Обнаружение** | Тестировщик, разработчик или пользователь видит ошибку. | Тестировщик / пользователь |
| **2. Создание отчёта** | Записывается в баг‑трекер: описание, шаги воспроизведения, окружение, приоритет. | Тестировщик |
| **3. Триаж (приоритезация)** | Команда оценивает важность и срочность. Принимается решение: фиксировать сейчас, отложить или закрыть как недоразумение. | Тестировщик + менеджер проекта |
| **4. Назначение** | Баг назначается разработчику (или группе) для исправления. | Менеджер проекта / Тестировщик |
| **5. Исправление** | Разработчик вносит код, делает ревью, коммитит. | Разработчик |
| **6. Проверка исправления** | Тестировщик повторно запускает тесты, проверяет, что баг действительно исправлен. | Тестировщик |
| **7. Закрытие** | Если баг закрыт, меняется статус на *Closed* в системе. | Тестировщик |
| **8. Пост‑мортем** | Анализ причины, какие улучшения можно внести в процессы, чтобы избежать повторов. | Вся команда |

### Ключевые моменты

- **Статусы** обычно: *New → Assigned → In Progress → Resolved → Verified → Closed*.
- **Приоритет vs. важность**: приоритет – как быстро нужно исправить, важность – насколько это критично для продукта.
- **Коммуникация**: все изменения статуса и комментарии фиксируются в баг‑трекере, чтобы все видели историю.

Такой цикл помогает быстро находить, исправлять и документировать ошибки, а также улучшать качество продукта.
</p>
    </details>
    <details style='margin-left: 20px'>
